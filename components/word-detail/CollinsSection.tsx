...
interface CollinsSectionProps {
    word: string;
    collinsPrimary?: CollinsPrimaryData;
    collinsOld?: CollinsData;
}

export const CollinsSection: React.FC<CollinsSectionProps> = ({ word, collinsPrimary, collinsOld }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            {primaryData.length > 0 && (
                <div id="collins_primary" className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="bg-amber-50/50 px-8 py-5 border-b border-amber-100 flex items-center justify-between">
                        ...
                        {oldEntries[0]?.star !== undefined && (
                            <div className="flex items-center bg-white px-3 py-1 rounded-full border border-amber-100 shadow-sm">
                                ...
                                <div className="flex">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} className={`w-3.5 h-3.5 ${i < (oldEntries[0].star || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-200'}`} />
                                    ))}
                                </div>
                            </div>
                        )}
            ...
            {/* Collins Old */}
            {oldEntries.length > 0 && (
                <div id="collins_old" className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="bg-amber-50/50 px-8 py-5 border-b border-amber-100 flex items-center justify-between">
                        <h3 className="font-bold text-amber-900 flex items-center text-lg">
