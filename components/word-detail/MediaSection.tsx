...
export const MusicSection: React.FC<{ musicSents?: MusicSentsData }> = ({ musicSents }) => {
    ...
    // Color Constants (Deep Violet Theme)
    const ACCENT_COLOR = 'text-violet-400';
    const BG_ACTIVE_COLOR = 'bg-violet-500';
    const HOVER_COLOR = 'hover:text-violet-300';
    ...
    // Explicit manual Next logic (ignores repeat-one)
        ...
        if (isShuffle && musicList.length > 1) {
            ...
            if (nextIndex >= musicList.length) {
                if (repeatMode === 'off' && !isShuffle) {
                    // Stop if list ends and no repeat
                    setIsPlaying(false);
    ...
    // Auto-advance logic (respects repeat-one)
    useEffect(() => {
    ...
    return (
        <div className={`bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative transition-all duration-500 ease-in-out ${isExpanded ? 'min-h-[600px] border-slate-900' : ''}`}>
            {/* Background for Expanded Mode */}
            {isExpanded && (
                <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 z-0"></div>
            )}

            {/* Header */}
            <div className={`relative z-10 flex items-center gap-2 px-8 py-5 border-b transition-colors duration-300 ${isExpanded ? 'border-white/5 bg-transparent' : 'border-slate-100 bg-violet-50/30'}`}>
                <Music className={`w-5 h-5 ${isExpanded ? 'text-violet-400' : 'text-violet-500'}`} />
                <h3 className={`text-lg font-bold transition-colors ${isExpanded ? 'text-white' : 'text-slate-800'}`}>原声歌曲</h3>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ml-auto transition-colors ${isExpanded ? 'text-violet-300 bg-white/10' : 'text-violet-500 bg-violet-50'}`}>
            ...
            {!isExpanded ? (
                ...
                <div className="animate-in fade-in slide-in-from-top-4 duration-300 relative z-10">
                    ...
                    <div className="relative w-full h-[320px] bg-slate-900 flex items-center justify-center overflow-hidden perspective-1000 group select-none">
                            ...
                            return (
                                <div key={idx} onClick={() => setActiveMusicIndex(idx)} className={`absolute w-48 h-48 sm:w-56 sm:h-56 rounded-xl shadow-2xl transition-all duration-500 ease-out cursor-pointer ${isActive ? 'z-30 ring-1 ring-white/20' : 'z-10 hover:opacity-80'}`} style={{ transform: `translateX(${xTranslate}%) scale(${scale}) perspective(1000px) rotateY(${rotateY}deg)`, zIndex, opacity, left: '50%', marginLeft: '-7rem' }}>
                                    <div className={`w-full h-full rounded-xl overflow-hidden bg-slate-800 relative border border-white/10 ${isActive && isPlaying ? 'animate-pulse-slow' : ''}`}>
                                        {(m.coverImg || m.cover) ? <img src={m.coverImg || m.cover} className="w-full h-full object-cover" /> : <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-violet-500 to-fuchsia-600"><Disc className="w-12 h-12 text-white/50" /></div>}
                        ...
                        {musicList.length > 1 && (<><button onClick={(e) => {e.stopPropagation(); playPrev();}} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition z-40 backdrop-blur-sm"><ArrowLeft className="w-6 h-6" /></button><button onClick={(e) => {e.stopPropagation(); playNext();}} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition z-40 backdrop-blur-sm"><ArrowRight className="w-6 h-6" /></button></>)}
                    </div>
                    ...
                    <div className="px-6 py-6 min-h-[150px] max-h-[300px] overflow-y-auto custom-scrollbar border-t border-slate-50 bg-slate-50/30">
                        <div className="text-center space-y-4">
                            <div className="mb-4">
                                ...
                                <div className="text-xs text-violet-500 font-medium flex items-center justify-center mt-1">
                            ...
                            <div className="text-slate-700 text-sm leading-relaxed font-serif whitespace-pre-wrap">
                                {activeMusic.lyric ? ( <div dangerouslySetInnerHTML={{__html: activeMusic.lyric}} /> ) : ( <p className="text-slate-400 text-xs italic py-4">暂无歌词预览</p> )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 backdrop-blur-md border-t border-slate-100 p-4 sticky bottom-0 z-20">
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-lg bg-slate-200 overflow-hidden shrink-0 border border-slate-200 ${isPlaying ? 'animate-spin-slow' : ''}`} style={{animationDuration: '10s'}}>
                                {(activeMusic.coverImg || activeMusic.cover) ? <img src={activeMusic.coverImg || activeMusic.cover} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-violet-100 flex items-center justify-center"><Music className="w-6 h-6 text-violet-300"/></div>}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                    <div className="truncate pr-4">
                                        <h4 className="text-sm font-bold text-slate-800 truncate">{activeMusic.songName || 'Unknown'}</h4>
                                        <p className="text-xs text-slate-500 truncate">{activeMusic.singer || 'Artist'}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={togglePlay} className="p-1.5 rounded-full hover:bg-slate-100 text-slate-700 transition">
                                            {isPlaying ? <PauseCircle className="w-6 h-6 fill-violet-50 text-violet-600"/> : <PlayCircle className="w-6 h-6 fill-slate-50 text-slate-600"/>}
                                        </button>
                                        <button onClick={() => setIsExpanded(true)} className="p-1.5 rounded-full hover:bg-slate-100 text-slate-500 transition" title="展开"><Maximize2 className="w-4 h-4"/></button>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono">
                                    <span>{formatTime(currentTime)}</span>
                                    <div className="flex-1 h-1 bg-slate-100 rounded-full relative group cursor-pointer">
                                        <div className="absolute inset-y-0 left-0 bg-violet-500 rounded-full" style={{ width: `${progress}%` }}></div>
                ...
                <div className="relative z-10 flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500 text-white">
                    <div className="flex-1 flex flex-col md:flex-row items-center justify-center gap-8 p-8 overflow-hidden min-h-[500px]">
                        ...
                        <div className="w-full md:w-1/2 flex items-center justify-center flex-col gap-8">
                            <div className={`relative w-[240px] h-[240px] md:w-[320px] md:h-[320px] rounded-full border-8 border-white/5 shadow-2xl overflow-hidden ${isPlaying ? 'animate-spin-slow' : ''}`} style={{animationDuration: '20s', boxShadow: '0 0 60px rgba(139, 92, 246, 0.15)'}}>
                                ...
                                {(activeMusic.coverImg || activeMusic.cover) ? 
                                    ...
                                    : <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-violet-900 to-fuchsia-900"><Music className="w-20 h-20 text-white/20"/></div>
                                }
                            ...
                            <div className="text-center space-y-2">
                                <h2 className="text-2xl md:text-3xl font-bold tracking-tight">{activeMusic.songName || 'Unknown Song'}</h2>
                                <p className="text-lg text-violet-200/60">{activeMusic.singer || 'Unknown Artist'}</p>
                            </div>
                        </div>
                                ...
                                {activeMusic.lyricList && activeMusic.lyricList.length > 0 ? (
                                    activeMusic.lyricList.map((line, lIdx) => (
                                        <div key={lIdx} className="space-y-1 group transition-all duration-500 hover:scale-105 opacity-60 hover:opacity-100">
                                            ...
                                            {line.lyricTranslation && <p className="text-sm md:text-base text-violet-200/50">{line.lyricTranslation}</p>}
                                        </div>
                                    ))
                    ...
                    {/* Footer Controls */}
                    <div className="bg-slate-950/50 backdrop-blur-md px-8 py-6 border-t border-white/10">
                        <div className="max-w-4xl mx-auto flex flex-col gap-4">
                            {/* Progress */}
                            <div className="flex items-center gap-4 text-xs font-mono text-violet-200/50">
                                <span>{formatTime(currentTime)}</span>
                                <div className="flex-1 h-1.5 bg-white/10 rounded-full relative cursor-pointer group">
                                    <div className={`absolute inset-y-0 left-0 rounded-full group-hover:bg-violet-300 transition-colors ${BG_ACTIVE_COLOR}`} style={{ width: `${progress}%` }}></div>
                                    <input type="range" min="0" max="100" value={progress} onChange={handleSeek} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                </div>
                            ...
                            <div className="flex items-center justify-center gap-10">
                                <button 
                                    ...
                                    className={`transition p-2 rounded-full hover:bg-white/10 ${isShuffle ? ACCENT_COLOR : 'text-white/40 hover:text-white'}`}
                                    title={isShuffle ? "随机播放: 开" : "随机播放: 关"}
                                >
                                    <Shuffle className="w-5 h-5"/>
                                </button>
                                
                                <button onClick={playPrev} className={`text-white ${HOVER_COLOR} transition p-2 hover:scale-110 active:scale-95`}><ArrowLeft className="w-8 h-8"/></button>
                                
                                <button 
                                    onClick={togglePlay} 
                                    className="w-16 h-16 rounded-full bg-white text-slate-900 flex items-center justify-center hover:scale-105 transition active:scale-95 shadow-lg shadow-white/10 hover:shadow-violet-500/20"
                                >
                                ...
                                <button onClick={playNext} className={`text-white ${HOVER_COLOR} transition p-2 hover:scale-110 active:scale-95`}><ArrowRight className="w-8 h-8"/></button>
                                
                                <button 
                                    onClick={toggleRepeat} 
                                    className={`transition p-2 rounded-full hover:bg-white/10 ${repeatMode !== 'off' ? ACCENT_COLOR : 'text-white/40 hover:text-white'}`}
                            ...
                            <div className="flex justify-center mt-2">
                                {(activeMusic.link || activeMusic.url) && (
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noreferrer" 
                                        className="flex items-center gap-2 px-6 py-2 bg-violet-600/20 hover:bg-violet-600/40 text-violet-200 rounded-full text-sm font-medium border border-violet-500/30 transition-all hover:scale-105"
